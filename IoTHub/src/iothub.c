...
static int IotHub_ReceiveDirectMethodCallback(const char* method_name, const unsigned char* payload, size_t size, unsigned char** response, size_t* resp_size, void* userContextCallback)
{
    ...
    *resp_size = strlen(RESPONSE_STRING);
    ...
    newMessageConfig.source = payload;
    newMessageConfig.size = size;
    ...
    else
    {
        ...
        else if (Map_Add(newProperties, "METHODNAME", method_name) != MAP_OK)
        {
            LogError("Property METHODNAME did not add properly");
            result = IOTHUBMESSAGE_ABANDONED;
        }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (IoTHubClient_SetDeviceMethodCallback(result->iothubHandle, IotHub_ReceiveDirectMethodCallback, result) != IOTHUB_CLIENT_OK)
                {
                    LogError("unable to IoTHubClient_SetDeviceMethodCallback");
                    IoTHubClient_Destroy(result->iothubHandle);
                    STRING_delete(result->deviceName);
                    STRING_delete(result->deviceKey);
                    free(result);
                ...
                else if (IoTHubClient_SetDeviceTwinCallback(result->iothubHandle, deviceTwinGetStateCallback, result) != IOTHUB_CLIENT_OK)
                {
                    LogError("unable to IoTHubClient_SetDeviceTwinCallback");
                    IoTHubClient_Destroy(result->iothubHandle);
                    STRING_delete(result->deviceName);
                    STRING_delete(result->deviceKey);
                    free(result);
                ...
                else
                {
                    ...
                    const char* reportedState = "{\"properties\":{\"reported\": {\"test\":3}}}";
                    size_t reportedStateSize = strlen(reportedState);
                
                    (void)IoTHubClient_SendReportedState(result->iothubHandle, (const unsigned char*)reportedState, reportedStateSize, deviceTwinReportStateCallback, result->iothubHandle);
